...
__global__ void globalMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    if (threadId < numThreads)
    {
        ...
        unsigned int partSize = numElements/numThreads;
        unsigned int startInputPart = threadId*partSize;
        unsigned int endInputPart = startInputPart + partSize;
        for (unsigned int idx = startInputPart; idx < endInputPart; idx++) {
            // Increment the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            if (constant_search_value == input[idx]) {
                input[idx] = 1; // in case this the search value was found, it should set the value to 1 like explained in the README
            }
        }
    }
}
...
{
    extern __shared__ int sharedInput[]; // only block local but therefore faster
    int threadId = blockDim.x * blockIdx.x + threadIdx.x;
    // every thread will load its own part into shared memory and after that it will continue with the search processing - this is more an educational example since we only have one block here
    ...
    if (threadId < numThreads)
    {
        ...
        for (unsigned int cpyIdx = startInputPart; cpyIdx < endInputPart; cpyIdx++) {
            sharedInput[cpyIdx] = input[cpyIdx];
        }
         __syncthreads();
        // Create a for loop that handles the fact that each thread needs to search for n values in input
        for (unsigned int idx = startInputPart; idx < endInputPart; idx++) {
            // Increment the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            if (constant_search_value == sharedInput[idx]) {
                input[idx] = 1; // this is the only access to global memory - therefor it will be faster then iterating over the global memory directly since shared memory is faster then global memory
            }
        }
    }
}
...
__global__ void constantMemorySearch(int totalFound)
{
    ...
    {
        // Create a for loop that handles the fact that each thread needs to search for n values in input
        unsigned int partSize = constant_num_elements/constant_num_threads;
        unsigned int startInputPart = threadId*partSize;
        unsigned int endInputPart = startInputPart + partSize;
        for (unsigned int idx = startInputPart; idx < endInputPart; idx++) {
            // Increment the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            if (constant_search_value == constant_input[idx]) {
                constant_input[idx] = 1; // in case this the search value was found, it should set the value to 1 like explained in the README
            }
        }
    }
}
...
{
    [[maybe_unused]] size_t size = numElements * sizeof(int);
    ...
    if(!strcmp(kernelType.c_str(), "constant"))
    {
        ...
        cudaMemcpyToSymbol(constant_input, input, numElements*sizeof(int), 0, cudaMemcpyHostToDevice); // offset to memory is 0 
    ...
    } else 
    {
        ...
        cudaError_t err;
        err = cudaMemcpy(d_input, input, numElements*sizeof(int), cudaMemcpyHostToDevice); // copy from CPU to GPU-global-memory
        if (err != cudaSuccess)
        {
...
__host__ void executeKernel(int *d_input, int numElements, int threadsPerBlock, std::string kernelType)
{
    ...
    {
        globalMemorySearch<<<1,threadsPerBlock>>>(d_input, totalFound, numElements, threadsPerBlock); // DONE: you will need to fill in function arguments appropriately
    } else if (!strcmp(kernelType.c_str(), "constant"))
    {
        constantMemorySearch<<<1,threadsPerBlock>>>(totalFound);  // DONE: you will need to fill in function arguments appropriately
    } else if (!strcmp(kernelType.c_str(), "shared"))
    {
        unsigned int_array_size = numElements * sizeof(int);
        sharedMemorySearch<<<1,threadsPerBlock, int_array_size>>>(d_input, totalFound, numElements, threadsPerBlock); // DONE: you will need to fill in function arguments appropriately
    } else {
        registerMemorySearch<<<1,threadsPerBlock>>>(d_input, totalFound, numElements, threadsPerBlock);  // DONE: you will need to fill in function arguments appropriately
    }
...
__host__ int * setUpInput(int numElements)
{
    ...
    cudaError_t err = cudaMemcpyToSymbol(constant_search_value, &searchValue, sizeof(int), 0, cudaMemcpyHostToDevice); // constant memory for the value to search - this is available in all threads on the GPU with high access efficency
    if (err != cudaSuccess)
    {
