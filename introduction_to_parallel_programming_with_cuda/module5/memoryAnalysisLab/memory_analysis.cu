...
{
    int threadId = blockDim.x * blockIdx.x + threadIdx.x; // this will be automatically placed within register memory by the compiler

    if (threadId < constant_num_threads)
    {
        [[maybe_unused]]double d = 47.11; // register memory as well
    }
}
...
__host__ int * allocateDeviceMemory(int numElements)
{
    ...
    int *d_input = NULL;
    cudaError_t err = cudaMalloc(&d_input, size); // global memory
    if (err != cudaSuccess)
    {
...
int main(int argc, char *argv[])
{
    ...
    int *input = setUpInput(numElements); // allocate constant memory on GPU for the search value
    int *d_input = allocateDeviceMemory(numElements); // allocate global memory for the data that should be searched through
