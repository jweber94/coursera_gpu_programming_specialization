/*
 * This is the code of the assignment with some comments that I made in order to understand the code better
 * 
...
__host__ tuple<int *, int *, int *> allocateRandomHostMemory(int numElements)
{
    ...
    int *h_a = (int *)malloc(size); // middle performance

    // Allocate the host pinned memory input pointer B
    int *h_b;
    cudaMallocHost((int **)&h_b, size); // high performance


    // Allocate the host mapped input vector C
    int *h_c;
    cudaMallocManaged((int **)&h_c, size); // low performance
...
__host__ tuple<int *, int *> allocateDeviceMemory(int numElements)
{
    ...
    int *d_b = NULL;
    err = cudaMalloc(&d_b, size); // for pinned memory, we need to allocate the corresponding memory on the GPU manually - if we would allocate mapped memory (pinned + memory mapped) we would not need to do this
    if (err != cudaSuccess)
    {
    ...
    // no need for the unified memory since the copy managing is done by the Cuda Runtime
...
__host__ void copyFromHostToDevice(int *h_a, int *h_b, int *d_a, int *d_b, int numElements)
{
    ...
    // Pinned memory copy
    err = cudaMemcpy(d_b, h_b, size, cudaMemcpyHostToDevice); // in case of mapped memory we would not need to do this
    if (err != cudaSuccess)
    {
...
__host__ void deallocateMemory(int *d_a, int *d_b)
{
    ...
    err = cudaFree(d_b); // this is also needed if we have had mapped memory
    if (err != cudaSuccess)
    {
