...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    float *h_a;
    cudaMallocHost((float **)&h_a, size, cudaHostAllocMapped);

    float *h_b;
    cudaMallocHost((float **)&h_b, size, cudaHostAllocMapped);
...
__host__ void copyFromHostToDevice(float *h_a, float *h_b, float *d_a, float *d_b, int numElements)
{
    ...
    cudaError_t err = cudaHostGetDevicePointer((float **)&d_a, (int *)h_a, size);
    ...
    err = cudaHostGetDevicePointer((float **)&d_b, (float *)h_a, size);
