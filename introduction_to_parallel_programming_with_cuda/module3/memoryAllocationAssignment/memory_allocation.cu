...
__host__ std::tuple<int *, int *> allocateRandomHostMemory(int numElements)
{
    ...
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_a needs to be pageable memory DONE
    cudaMallocHost((int**)&h_a, size, cudaHostAllocPortable); // cudaHostAllocPortable is pinned memory according to https://developer.download.nvidia.com/compute/DevZone/docs/html/C/doc/html/group__CUDART__MEMORY_g15a3871f15f8c38f5b7190946845758c.html 
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_a needs to be pageable memory DONE

    // Allocate the host pinned memory input pointer B
    int *h_b;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_b needs to be pinned memory DONE
    cudaMallocHost((int**)&h_b, size, cudaHostAllocPortable);
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_b needs to be pinned memory DONE
...
__host__ std::tuple<int *, int *> allocateDeviceMemory(int numElements)
{
    ...
    size_t size = numElements * sizeof(int);
    cudaError_t err;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_a needs to handle pageable memory and copies DONE
    ...
    int *d_b;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_b needs to handle pinned memory and copies DONE
...
int main(int argc, char *argv[])
{
    ...
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_c needs to be unified memory DONE
    int *h_c;
    cudaMallocManaged((void**)&h_c, sizeof(int)*numElements);
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE - SPECIFICALLY h_c needs to be unified memory DONE
