...
{
    // REMARK: This is the only function that is executed on the GPU via the runtime API of cuda
...
__host__ void copyFromHostToDevice(int h_v, int *h_d, int *d_d, int numElements)
{
    ...
    err = cudaMemcpyToSymbol(d_v, &h_v, sizeof(int), 0, cudaMemcpyHostToDevice); // this will copy to the aimed adress d_v with the offset 0 (https://developer.download.nvidia.com/compute/DevZone/docs/html/C/doc/html/group__CUDART__MEMORY_gf268fa2004636b6926fdcd3189152a14.html)
    if (err != cudaSuccess)
    {
 ...
 * d = data to search in
 * h_i = input from the GPU result - this is a chunk of memory where the result of the GPU calculation will be placed after the GPU has finished its job 
 */
int main(int argc, char *argv[])
{
    srand(time(0));
    int numElements = 10;
    int h_v = -1; // value v on host
    int *h_d; // data on host
    ...
    // calculation result init on host (to this place we want to copy the calculation result from the GPU to the CPU)
    ...
    auto[d_d, d_i] = allocateDeviceMemory(numElements); // d_d is the data array on the device (aka GPU)
