...
{
    int i = blockDim.x * blockIdx.x + threadIdx.x; // blockIdx. is a dim3 datastructure

    if (i < numElements) // validity check if we have more threads then data handed over to the GPU call
    {
        int inputVal = d_d[i];
        if(inputVal == d_v)
        {
            d_i[0] = i; // write index to the result vector
        }
    }
}
...
__host__ void copyFromHostToDevice(int h_v, int *h_d, int h_i, int *d_d, int *d_i, int numElements)
{
    ...
    err = cudaMemcpyToSymbol(d_v, &h_v, sizeof(int), 0, cudaMemcpyHostToDevice); // d_v is defined in the search.h
    if (err != cudaSuccess)
    {
...
int main(int argc, char *argv[])
{
    ...
    h_d = get<0>(searchInputTuple); // host data to search in
    numElements = get<1>(searchInputTuple);
    h_v = get<2>(searchInputTuple); // host value to search

    auto[d_d, d_i] = allocateDeviceMemory(numElements); // allocate memory on GPU for the data and the result of the calculation - the result is just one int big since we are looking only for one number in the search field (aka h_d (host_data) which is copied to d_d (device_data))
    copyFromHostToDevice(h_v, h_d, h_i, d_d, d_i, numElements); // d_i is d_v
