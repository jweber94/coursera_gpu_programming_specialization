/*
 * CAUTION: This is my personal solution to the coding assignment of module 2. It is the copied version but with my own
 * solution to calculate the gray scale image from the input image. The input image is not included here, so you need to place
 * your own image as "sloth.png" for the input. 
 *
...
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    if (threadId < (d_columns * d_rows)) { // Do the clamping
        d_gray[threadId] = static_cast<uchar>((d_r[threadId] + d_g[threadId] + d_b[threadId])/3); // FIXME: Maybe we want to cast temporarily into a double to have a more accurate result by dividing by 3
    }
}
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray, int rows, int columns, int threadsPerBlock)
{
    ...
    dim3 block(1, threadsPerBlock, blockZSize); // 1*?*4 threads per block
...
__host__ std::tuple<int, int, uchar *, uchar *, uchar *> readImageFromFile(std::string inputFile)
{
    ...
    Mat img = imread(inputFile, IMREAD_COLOR); // read file with opencv
    ...
    // create host memory on heap for the image
    ...
    // write the individual color channels to the host heap memory
    for(int r = 0; r < rows; ++r)
    {
        for(int c = 0; c < columns; ++c)
        {
...
int main(int argc, char *argv[])
{
    ...
    try 
    {
        ...
        uchar *gray = (uchar *)malloc(sizeof(uchar) * rows * columns);
        std::tuple<uchar *, uchar *, uchar *, uchar *> memoryTuple = allocateDeviceMemory(rows, columns); // allocate the corresponding memory on the GPU to be able to copy the image (as well as the resulting grey value image) to and from the device (GPU)
        uchar *d_r = get<0>(memoryTuple);
        uchar *d_g = get<1>(memoryTuple);
        uchar *d_b = get<2>(memoryTuple);
        uchar *d_gray = get<3>(memoryTuple); // result pointer to memory on GPU
        ...
        // get the result back from the GPU and save the data on the host machine
        copyFromDeviceToHost(d_gray, gray, rows, columns);
        
        /// cleanup and evaluate the image
