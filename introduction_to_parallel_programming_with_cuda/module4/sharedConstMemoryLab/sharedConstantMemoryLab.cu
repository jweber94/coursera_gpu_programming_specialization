...
__global__ void kernel(uchar *d_r, uchar *d_g, uchar *d_b)
{
    ...
    // IDEA: If we want to use a convolution kernel to process over the image, we could allocate 
    // constant memory for the convolution filter kernel (e.g. 3x3, 5x5 or 7x7) and use it 
    // within all threads. This would speed up the calculation

    // IDEA: We also could use shared memory for blocks to load a pixel neighbourhood to avoid that much of a 
    // loading of global memory. This also makes the convolution kernel replaceable by the application program
...
__host__ std::tuple<uchar *, uchar *, uchar *> allocateDeviceMemory(int rows, int columns)
{
    ...
    //Allocate device constant symbols for rows and columns - write to the constant memory on the GPU
...
__host__ std::tuple<int, int, uchar *, uchar *, uchar *> readImageFromFile(std::string inputFile)
{
    ...
    for(int r = 0; r < rows; ++r)
    {
        ...
        {
            // idea: to do a 2D convolution for a e.g. gaussian smoothing, we would add a 1 pixel 
            // padding to the top row as well as the first an last element of the colum while filling 
            // up the h_r, h_g and h_b and start the kernel at the individual offsets. This would result 
            // in a modelling based on the blocks and grid which differenciates the rows and cols
...
int main(int argc, char *argv[])
{
    ...
    try 
    {
        ...
        // result memory reservation on CPU/Host 
        ...
        Mat outputImageMat(rows, columns, CV_8UC1); // 8UC1 is a 256 value grey scale image in opencv
