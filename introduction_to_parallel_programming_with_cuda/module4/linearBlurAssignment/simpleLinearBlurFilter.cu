...
__global__ void applySimpleLinearBlurFilter(uchar *r, uchar *g, uchar *b)
{
    ...
    /// REMARK: since the .y and .z dimensions of the block and grid are implicitly set to 1, we do not need to consider them here
    unsigned int threadId = threadIdx.x + blockIdx.x*blockDim.x;
    //unsigned int blockOffset = blockIdx.x * blockDim.x;

    if(threadId < d_columns*d_rows)
    {
        // When using shared memory you should store pixel values relevant to the current thread in a variable
        /// Creating static memory arrays that lasts throughout the block lifetime
        __shared__ uchar blockR[256]; /// FIXME: Make this dynamic shared memory since threadsPerBlock might change dynamically
        __shared__ uchar blockG[256];
        __shared__ uchar blockB[256];
        /// copy the block pixels to the shared memory - every thread copies its own block pixel and after that we sync
        blockR[threadIdx.x] = r[threadId];
        blockG[threadIdx.x] = g[threadId];
        blockB[threadIdx.x] = b[threadId];
        // sync threads so that you can alter RGB values without causing race condition
        __syncthreads(); /// REMARK: sync threads to avoid that one thread is writing to the row while it is copied into shared memory - only after we copied into shared memory we can manipulate the row in the handedover channel arrays
        ...
        /// TODO: do the calculation and assignment to the *r, *g and *b
        bool firstColElement = (0 == (threadId % d_columns));
        bool lastColElement = (0 == (threadId % (d_columns - 1)));
        if (firstColElement || lastColElement) {
            return; // do not change the pixel value as in the applyBlurKernel function on CPU
        }
        r[threadId] = blockR[threadId - 1] + blockR[threadId] + blockR[threadId+1]/3;
        g[threadId] = blockG[threadId - 1] + blockG[threadId] + blockG[threadId+1]/3;
        b[threadId] = blockB[threadId - 1] + blockB[threadId] + blockB[threadId+1]/3;
    }
}
...
__host__ void executeKernel(uchar *r, uchar *g, uchar *b, int rows, int columns, int threadsPerBlock)
{
    ...
    applySimpleLinearBlurFilter<<<blocksPerGrid, threadsPerBlock>>>(r, g, b); // grid is blocksPerGridx1x1 and blocks are threadsPerBlockx1x1 since they cast implicitly to dim3
...
__host__ std::tuple<uchar *, uchar *, uchar *>applyBlurKernel(std::string inputImage)
{
    ...
    for(int y = 0; y < rows; ++y)
    {
        ...
        {
            // CAUTION: This is a trivial bluring by averaging the values of the previous and the next pixel in the row (x_minus_1 + x * x_plus_1) = x_new
