...
__host__ float *allocateHostMemory(int numElements, int seed)
{
    ...
    /// pinned memory is always contiguous memory in RAM on the host
    /// since it is contiguous and will not be paged out, the copy process
    /// is much faster then copying pageble memory from the host to the device
    /// CAUTION: it will not automatically be available on the GPU, you need to cudaMemcpy
    /// it. If you want automatic copies, you need to use cuda managed memory (aka managed memory via cudaMallocManaged(...))
...
__host__ float * allocateDeviceMemory(int numElements)
{
    ...
    float *dev_mem = NULL;
    size_t size = numElements * sizeof(float);
    cudaError_t err = cudaMalloc(&dev_mem, size); // allocation of global memory on GPU
    if (err != cudaSuccess)
    {
...
{
    /// Copying is still needed even if we use pinned memory on the host. This will only accellerate the copying process 
    /// but will not eliminate the copying itself - if you want this, you need to use mapped cuda memory (aka cuda managed memory)
    size_t size = numElements * sizeof(float);
    // Copy the host input vector to the device input vectors
    printf("Copy input data from the host memory to the CUDA device\n");
...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    auto[threadsPerBlock, blocksPerGrid] = determineThreadBlockDimensions(num_elements); // only x dimension of the grid and block are set
    ...
    // does the 6th stream miss here

    // Copy device memory back to host asynchronously
    copyFromDeviceToHostAsync(dev_mem, host_mem, num_elements, stream6); // wait for stream 6 to copy
