 ...
 */
 ...
 __global__ void compare(const int *a, const int *b, int *c, int numElements)
 {
     ...
     if (i < numElements)
     {
         //c[i] = (a[i] > b[i])*1 + (a[i] < b[i])*(-1) + ((a[i] == b[i])*0);  
         if (a[i] > b[i]) {
             c[i] = 1;
         } else if (a[i] == b[i]) {
             c[i] = 0;
 ...
 __host__ std::tuple<int *, int *, int *> allocateHostMemory(int numElements)
 {
     ...
     int *h_c;
     cudaMallocManaged((int **)&h_c, size); // CAUTION: We let cuda runtime api manage the memory exchange for us - this is convenient to use as a result pointer but is the slowest version for the data exchange regarding efficency
 ...
 __host__ void executeKernel(int *d_a, int *d_b, int *c, int numElements)
 {
     ...
     compare<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, c, numElements); /// Grid and block dimension are Nx1x1 and Mx1x1 so we only use the x dimension of the thread execution model
 ...
 __host__ void deallocateMemory(int *h_a, int *h_b, int *h_c, int *d_a, int *d_b)
 {
     ...
     auto error = cudaFree(h_c);
     if (error != cudaSuccess) {
         fprintf(stderr, "cudaFreeManaged failed: %s \n", cudaGetErrorString(error));
         exit(EXIT_FAILURE);
     }
 }
 ...
 __host__ void performMultiCPUIteration()
 {
     ...
     std::tuple<int *, int *, int *> hostPtrs = allocateHostMemory(numElements);
     int* h_a = std::get<0>(hostPtrs);
     int* h_b = std::get<1>(hostPtrs);
     int* c = std::get<2>(hostPtrs); // cuda runtime managed memory - can be used on GPU as well as on CPU with the same pointer
     
     std::tuple<int *, int *> devicePtrs = allocateDeviceMemory(numElements);
     int* d_a = std::get<0>(devicePtrs);
     int* d_b = std::get<1>(devicePtrs);
     ...
     /* Explaination: The lock files work as locks to be acquired - if the lock file exists it can be acquired by a process */
     // ACQUIRE LOCK: Remove locks on input and lock on output, which should signal consumers to read
     //remove( "./input_a.lock" ); // this is basically the acquirement of the lock file
     //remove( "./input_b.lock" );
     
     // Create output lock files to signal python processes to print out data and generate input data
     std::fstream output_fstream_a;
     output_fstream_a.open("output_a.lock", std::ios_base::out); // if the file does not exist before, this is the release of the lock file from this process
     output_fstream_a.is_open(); // check if it worked - if not we will throw an exception here
     std::fstream output_fstream_b;
     output_fstream_b.open("output_b.lock", std::ios_base::out);
     output_fstream_b.is_open(); // check if it worked - if not we will throw an exception here
     
     std::fstream output_fstream_input_a;
     output_fstream_input_a.open("input_a.lock", std::ios_base::out); // if the file does not exist before, this is the release of the lock file from this process
     output_fstream_input_a.is_open();
     std::fstream output_fstream_input_b;
     output_fstream_input_b.open("input_b.lock", std::ios_base::out); // if the file does not exist before, this is the release of the lock file from this process
     output_fstream_input_b.is_open();
 
     // deallocate memory
     deallocateMemory(h_a, h_b, c, d_a, d_b);
 }
