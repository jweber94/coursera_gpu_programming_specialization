 ...
 */
 ...
 __host__ void retrieveDataFromFiles(float *h_a, float *h_b, int numElements)
 {
     ...
     /// waiting until the python producer creates the lock file which indicates that the input data was completly and successfull written to the input csv file
     while(!locksExist)
     {
     ...
     /// remove the old output data (from the GPU processing) to stop the python application to read in the data
     ...
     /// since the input lock exists (which was checked above), we can read out the data and process it
