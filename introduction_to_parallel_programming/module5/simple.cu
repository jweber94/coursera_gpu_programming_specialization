/* This is a copy of the assignment of the keyword labor of module 5 but with my solution to the assignment */
 ...
 */
 ...
 __global__ void vectorMult(const float *A, const float *B, float *C, int numElements)
 {
     ...
     if (i < numElements)
     {
         C[i] = deviceMultiply(A[i], B[i]); // calls a function from the gpu on the gpu which means __device__ keyword
     }
 }
 
 __device__ float deviceMultiply(float a, float b) // added device keyword
 {
     return a * b;
 }
 ...
 __host__ std::tuple<float *, float *, float *> allocateDeviceMemory(int numElements) // no __global__ keyword since we do not execute this on the GPU but on the CPU - it is a non- or __host__ keyword
 {
 ...
 __host__ void copyFromHostToDevice(float *h_A, float *h_B, float *d_A, float *d_B, int numElements) // no __global__ keyword since we do not execute this on the GPU but on the CPU - it is a non- or __host__ keyword
 {
     size_t size = numElements * sizeof(float);
     // Copy the host input vectors A and B in host memory to the device input vectors in device memory
     printf("Copy input data from the host memory to the CUDA device\n");
     cudaError_t err = cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToDevice); // cudaMemcpy(destination, source, from/toGpu)
     if (err != cudaSuccess)
     {
