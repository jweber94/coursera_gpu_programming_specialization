...
# Solution to the example made by Jens Weber
'''
Needed resources:
...
https://stackoverflow.com/questions/74405721/python-threading-event-object-how-to-notify-specific-thread
'''
...
class Customer(threading.Thread):       # Producer Thread
    ...
    def trim(self):
        ...
        # DONE execute the time sleep function with a, which simulates the time it takes for a barber to give a haircut.
        time.sleep(a)
        payment = self.rate
...
class Barber(threading.Thread):     # Consumer Thread
    def __init__(self, queue):      # Constructor passes Global Queue (all_customers) to Class
        threading.Thread.__init__(self)
        # DONE set this class's queue property to the passed value
        self.queue = queue
    ...
    def run(self):
        ...
        while SHOP_OPEN:
            while self.queue.empty():
                ...
                ### DONE This is the bugfix
                EVENT.wait(1)  # wait with timeout
                if (not SHOP_OPEN):
                    return
            ...
            customer = self.queue
            # DONE use the task_done function to complete cutting the customer's hair
            customer.task_done()
...
if __name__ == '__main__':
    ...
    for b in range(BARBERS):
        # DONE Pass the all_customers Queue to the Barber constructor
    ...
    for c in range(10):  # Loop that creates infinite Customers
        ...
        c.run()
        # DONE Invoke the run method in the Customer Class
